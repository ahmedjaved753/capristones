...
- **Natural Stone listing** (`/natural-stone`) — grid/gallery view of 25 products served from Supabase Storage. Filters (type, color, finish) and search were removed on 2026-04-29 because product metadata is still placeholder; they'll come back when the client supplies real product types and names. (Origin filter was removed on 2026-04-28; all stone is sourced from Brazil.)
- **Quartz listing** (`/quartz`) — grid/gallery with search + type/color/finish filters (still on placeholder mock data; the pattern Natural Stone used until 2026-04-29).
